...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const bodySearchForm = document.getElementById('body-search-form');
  const resultsContainer = document.getElementById('search-results');

  function performSearch() {
    // Collect form fields
    const formData = new FormData(bodySearchForm);
      ...
      .then(data => {
        ...
        if (data.data && data.data.length > 0) {
          data.data.forEach(vehicle => {
            html += `
              <div class="vehicle-result border p-3 mb-3">
                <h4>${vehicle.make_id ? 'Make ID: ' + vehicle.make_id : ''} 
                    ${vehicle.model || ''} 
                    ${vehicle.year ? '(' + vehicle.year + ')' : ''}</h4>
                <p>${vehicle.description || ''}</p>
                <p><strong>VIN:</strong> ${vehicle.vin} 
                   | <strong>Lot#:</strong> ${vehicle.lot_number || 'N/A'}</p>
                <p><strong>Auction Date:</strong> 
                   ${vehicle.auction_date ? new Date(vehicle.auction_date).toLocaleDateString() : 'N/A'}</p>
              </div>
            `;
          });
        } else {
  ...
  bodySearchBtn.addEventListener('click', function(e) {
    e.preventDefault();
    performSearch();
  });

  // Optional: if user presses Enter in the form
  bodySearchForm.addEventListener('submit', function(e) {
    e.preventDefault();
    performSearch();
  });
});
